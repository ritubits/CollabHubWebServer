...
public class AllowedCollaboratorServlet extends HttpServlet{
    ...
    String DEBUG=null;
	 ...
	 public int getAllowedCollaborators(Connection con)
	  {
		 System.out.println("In AllowedCollaboratorServlet::getAllowedCollaborators");
		 int allowedNo=0;
		  ...
		  try {
		       if (con !=null) 
		    	   ...
		    	   // Result set get the result of the SQL query
					sql= "select allowedCollab from regproject where projectName ='"+projectName+"'";
	 ...
	 public int getCurrentCollaborators(Connection con)
	  {
		 System.out.println("In AllowedCollaboratorServlet::getCurrentCollaborators");
		 int currentNo=0;
		Statement statement = null;
			  String sql = null;
			 ResultSet resultSet =null;
		  try {
		       if (con !=null) 
		    	   { 
					statement = con.createStatement();				    	   
		    	   // Result set get the result of the SQL query
					System.out.println("select * from userdetails_"+projectName+ " where action='COLLAB_STARTED'");
					sql= "select * from userdetails_"+projectName+ " where action='COLLAB_STARTED'";
					resultSet = statement.executeQuery(sql);
					while (resultSet.next())
					{
						resultSet.last();
