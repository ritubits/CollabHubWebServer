...
public class CreateDependencyGraph {
	 ...
	 * 
	 * for (int i=0; i<=4; i++) { db.initializeDB("CollabProject"); try {
	 * Thread.sleep(40000); } catch (InterruptedException e) { // TODO
	...
	public void initializeDB(String pName, String srcPath) {
		...
		try {
			...
			GraphDatabaseBuilder graphBuilder = graphFactory
					...
					if (isInterface) {
						...
						if (packageNode != null)
							cNode = dpGraph.addConnectingClassNode(graphDb,
									packageNode, smallClassName, className, cu
											.imports().toString(), packName,
									modifier, extend, implemented, lineNumber);
						else
							cNode = dpGraph.addConnectingClassNode(graphDb,
									rootNode, smallClassName, className, cu
											.imports().toString(), packName,
						...
						for (VariableDeclarationFragment fd : frag) {
							...
							if (o instanceof VariableDeclarationFragment) {
								...
								aNode = dpGraph.addAttributeNode(graphDb,
										...
										attributeType, initializer, lineNumber);
							} // if
					...
					if (superClass != null) {
						...
						if (idSuperNode != -1) {
							...
							dpGraph.addExtendsDependencyEdge(graphDb,
									idSuperNode, idCurrentClassNode);
						}
					}// if (superClass !=null)
						...
						for (Type interfaces : interfacesImplemented) {
							...
							if (idinterfaceNode != -1) {
								...
								dpGraph.addImplementsDependencyEdge(graphDb,
										idinterfaceNode, idCurrentClassNode);
							}// if (idinterfaceNode != -1)
						...
						while (enumImports.hasMoreElements()) {
							...
							if (importClassId != -1) {
								...
								dpGraph.addImportsDependencyEdge(graphDb,
										importClassId, idCurrentClassNode);
							}// if (idinterfaceNode != -1)
						}
	...
	public void createAttributeDependencyGraph() {
		...
		ResourceIterator<Node> aNodes = graphDb
				.findNodes(dGraphNodeType.ATTRIBUTE);
	...
	public void createMethodAttributeDependencyGraph() {
		...
		Iterable<Relationship> relations;
		ResourceIterator<Node> methodANodes = graphDb
		...
		cu.accept(new ASTVisitor() {
			...
			public boolean visit(MethodDeclaration node) {
				...
				Node mNode = dpGraph.addMethodNode(graphDb, cNode,
						...
						returnTypeString, param, methodBody, lineNumber);
				if (node.getBody() != null)
	...
	public void visitFileAST(final dependencyGraphNodes dpGraph,
		...
		cu.accept(new ASTVisitor() {
			...
			public boolean visit(MethodInvocation node) {
				...
				Long invokeMethodNodeId = searchHashNode(invokedMethodName,
						nodeCanonicalHashMap);
				// Long currentMethodNodeId = searchNode(graphDb,
				// dGraphNodeType.METHOD, "canonicalName", currentMethodName);
				Long currentMethodNodeId = searchHashNode(currentMethodName,
						nodeCanonicalHashMap);
