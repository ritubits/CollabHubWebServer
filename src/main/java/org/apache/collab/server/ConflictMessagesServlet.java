...
/**
 * This is the servlet that provides reads, filters and passes the conflict
 * messages to the collaboratorsConflictsViewParts plugin of the
 * COGEclipseClient.
 * 
 * @author Ritu Arora
 * 
...
public class ConflictMessagesServlet extends HttpServlet {
	...
	public void getActivtyData(String collabName) {
		...
		try {
			if (con != null) {
				...
				sql = "select filename, activitytype, max(activitytime) from (select * from useractivity_"
						+ collabName
						+ " order by filename ASC, activitytime DESC) as t2 group by filename;";
	...
	public String getConflictMessages(Connection con, String collabName) {
		...
		try {
			if (con != null) {
				...
				statement = con.createStatement();
				// Result set get the result of the SQL query
				sql = "select * from conflictmessages where collabName <> '"
						+ collabName + "' order by messagetime DESC";
				resultSet = statement.executeQuery(sql);
				while (resultSet.next()) {

					sentNode = resultSet.getString("sentNode");
					message = resultSet.getString("message");
					sentCollab = resultSet.getString("collabName");// message
																	// sent by
																	// this
																	// collaborator
					if (artifactFound(sentNode)) {
						if (artifactName == null)
							...
							artifactName = artifactName + "," + sentNode;
	...
	public String getColorString(String sentNode) {
		...
		try {
			if (con != null) {
				...
				statement = con.createStatement();
				// Result set get the result of the SQL query
				sql = "select filename, activitytype, activitytime from useractivity_"
						+ collabName
				...
				while (resultSet.next()) {
					...
					System.out.println("from getColorString fileName::"
							+ fileName);
					index2 = fileName.indexOf(".");
					fName = fileName.substring(0, index2);
					if (DEBUG.contains("TRUE"))
						System.out.println("from getColorString sentNode::"
								+ sentNode);
					if (DEBUG.contains("TRUE"))
						System.out.println("from getColorString fileName::"
								+ fName);
	...
	public String getCollabType(String sentCollab) {
		...
		try {
			if (con != null) {
				...
				statement = con.createStatement();
				sql = "SELECT table_name FROM information_schema.tables	WHERE table_schema = 'collaborationhub'	AND table_name = 'useractivity_"
						+ collabName + "'" + ";";
			...
			if ((myFileName != null)
					&& (myFileName.equalsIgnoreCase(collabFileName))) {
				color = "EDC";
			} else
				color = "EIC";
	...
	public boolean artifactFound(String artifactName) {
		// This method checks if the sentNode is the same as the current edit
		// artifact
		...
		if (getActivityArtifact() != null) {
			if (getActivityArtifact().contains(artifactName))
				found = true;
		}
