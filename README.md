...
•	InconsistencyCommunicator Component: This component converts conflict messages into messages to be sent to collaborating clients (using rules for identifying clients listed in Table 5.4, Chapter 5) (InconsistencyCommunicator.java). These messages are stored in the MySQL database which are then retrieved and display by all collaborating clients.
